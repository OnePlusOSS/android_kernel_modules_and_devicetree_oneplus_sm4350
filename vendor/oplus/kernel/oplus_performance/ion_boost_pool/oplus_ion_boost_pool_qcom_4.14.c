...
#define NR_MAX_KILL (3)
static void boost_pool_direct_kill(unsigned long reclaim_by_kill)
{
	...
	for_each_process(tsk) {
		...
		if (oom_score_adj < 701) {
			task_unlock(p);
			continue;
		}

		task_size = get_mm_counter(p->mm, MM_SWAPENTS) / 3 +
			get_mm_counter(p->mm, MM_ANONPAGES);

		task_unlock(p);

		if (task_size <= 0 || task_size < (SZ_32M >> PAGE_SHIFT))
			continue;

		task_lock(p);
		send_sig(SIGKILL, p, 0);
		pr_info("crit scene kill '%s' (tgid %d), adj %hd, size %d\n",
			p->comm, p->tgid, oom_score_adj,
			task_size);
		nr_kill++;
		task_unlock(p);
		kill_size += task_size;
		if (kill_size >= reclaim_by_kill || nr_kill >= NR_MAX_KILL)
			break;
	}
				     ...
				     size_t count, loff_t *ppos)
{
	...
	if (mutex_trylock(&boost_pool->prefill_mutex)) {
		...
		long safe_pages = (SZ_2G + SZ_512M) >> PAGE_SHIFT;
		...
		if (mem_avail < safe_pages)
			boost_pool_direct_kill(safe_pages - mem_avail);
